...
class CustomTopo(Topo):
    def __init__(self, **opts):
        ...
        host_2 = self.addHost("h2", ip="10.2.0.5")
...
for src, dst in [
    ...
    if src == "h1" and dst == "s0":
        ...
        sw.insertTableEntry(
            ...
            match_fields={"hdr.ipv4.dstAddr": ["10.5.0.2", 32]},
        ...
        sw.insertTableEntry(
            ...
            match_fields={"hdr.ipv4.dstAddr": ["10.5.0.2", 32]},
            action_name="MyIngress.srv4_next_c_sid_swap",
        )
        sw.insertTableEntry(
            table_name="MyIngress.ipv4_lpm",
            match_fields={"hdr.ipv4.dstAddr": ["10.2.0.5", 32]},
            action_name="MyIngress.ipv4_forward",
...
# Set ARP entry for host h1
net.get("h1").cmd("arp -s 10.5.0.2 %s" % net.get("s0").intfs[1].MAC())

# Ping from host h1 to IP 10.5.0.2
print(net.get("h1").cmd("ping -c 1  10.5.0.2"))